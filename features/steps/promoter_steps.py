...
def write_service_to_int_git(context):
    ServiceVersionWriter(GIT_REPO_URL).write('kuku', JAVA_SERVICE_NAME, JAVA_SERVICE_IMAGE_NAME)
...
def check_promoted_service_in_git(context):
    assert ServiceVersionReader(GIT_REPO_URL).read(TARGET_ENV)[0] == JAVA_SERVICE_IMAGE_NAME
